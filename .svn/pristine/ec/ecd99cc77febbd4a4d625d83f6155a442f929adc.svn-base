...
#   predPC <- valP
#
# Improve of PIPE HCL code:
# 1.
#   Change:
#   Forward m_valM from Memory to Excute when there's a mrmovl in 
#   Memory and a rmmovl in Excute and they have the same register 
#   to operate, because rmmovl does nothing in Excute, doing this
#   can decrease a bubble in each operation.
#   Method:
#   Modified the HCL code of e_valA and Pipeline Register Control.
#
# 2.
#   Change:
#   Make jle jump to the right instruction when there'a andl rA,rA
#   before it. It can decrease 2 bubbles in an wrong prediction. If
#   this occasion occurs, d_dstM will be set to 8 as a symbol, which 
#   will be recovered in write back stage.
#   Method:
#   Modified the HCL code of f_pc, d_dstM, w_valM and Pipeline 
#   Register Control.
#   
####################################################################
...
intsig D_ifun 'if_id_curr->ifun'	 # ifun
...
int f_pc = [
	...
	D_icode == IJXX && D_ifun == 1 &&
	E_srcA == E_srcB && E_valA > 0 : D_valP; 
	# Mispredicted branch.  Fetch at incremented PC
	M_icode == IJXX && !M_Cnd && M_dstM != 8 : M_valA;
...
int e_valA = [
	M_icode == IMRMOVL && E_icode in { IRMMOVL, IPUSHL } && E_srcA == M_dstM : m_valM;
...
bool F_stall =
	...
	E_icode in { IMRMOVL, IPOPL } &&
	 ...
	 !(E_icode == IMRMOVL && D_icode in { IRMMOVL, IPUSHL }) ||
