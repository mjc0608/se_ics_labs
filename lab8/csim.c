...
void print2(unsigned long long x) {
...
void run_cache(int display_trace_inf, int associativity, int set_index_bits, int block_bits, char* tracefile) {
    ...
    block_mask=(((unsigned long long)1)<<block_bits)-1;print2(block_mask);
    set_mask=((((unsigned long long)1)<<(set_index_bits+block_bits))-1)^block_mask;print2(set_mask);
    tag_mask=((unsigned long long)(-1))^(set_mask|block_mask);print2(tag_mask);
    ...
    while (fscanf(trace,"%c%llx,%d\n",&op,&address,&length)!=EOF) {
