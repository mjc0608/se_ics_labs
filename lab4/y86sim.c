...
stat_t nexti(y86sim_t *sim)
{
    ...
    long_t valC = 0;
    long_t valA = 0, valB = 0, valE = 0, valM = 0;
    ...
    if (icode!=I_HALT && icode!=I_NOP && icode!=I_JMP && icode!=I_CALL && icode!=I_RET) {
        if (!get_byte_val(sim->m, next_pc, &reg2)) {
            err_print("PC = 0x%x, Invalid instruction address", sim->pc);
            return STAT_ADR;
        }
        regA=GET_REGA(reg2);
    ...
    if ((regA<0 || (regA>REG_CNT && regA!=REG_NONE)) || (regB<0 || (regB>REG_CNT && regB!=REG_NONE))) {
        err_print("PC = 0x%x, Invalid register(s) %.2x", sim->pc, reg2);
        return STAT_INS;
    }
    ...
    switch (icode) {
        ...
        case I_CALL: /* 8:x imm */
            ...
            if ((unsigned)valE>=(unsigned)MEM_SIZE) {
